        ...
        <>
          <div style={styles.tabs}>
            {['client','freelancer','arbitrator'].map(r => (
              <button key={r} style={{...styles.tab, ...(role===r ? styles.activeTab : {})}}
                onClick={() => setRole(r)}>
...
const styles = {
  app: { maxWidth: '800px', margin: '0 auto', padding: '2rem', fontFamily: 'sans-serif', background: '#f8fafc', minHeight: '100vh' },
  tabs: { display: 'flex', gap: '0.5rem', marginBottom: '1.5rem' },
  tab: { padding: '0.6rem 1.5rem', border: '1px solid #d1d5db', borderRadius: '8px',
    background: '#fff', cursor: 'pointer', fontSize: '0.95rem', color: '#374151' },
  activeTab: { background: '#4f46e5', color: '#fff', border: '1px solid #4f46e5' },
  content: { marginTop: '0.5rem' }
};
