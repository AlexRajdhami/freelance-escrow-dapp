...
function ArbitratorPanel({ provider }) {
  ...
  const resolve = async (releaseToFreelancer) => {
    try {
      ...
      await tx.wait();
      setStatus(releaseToFreelancer ? 'Resolved: funds sent to freelancer!' : 'Resolved: funds refunded to client!');
    } catch (err) {
      setStatus(`Error: ${err.message}`);
    } finally {
      setLoading(false);
  ...
  return (
    <div style={styles.panel}>
      <h2 style={styles.heading}>Arbitrator Panel</h2>
      ...
      {escrowInfo && (
        <div style={styles.infoBox}>
          ...
          <p><strong>Freelancer:</strong> {escrowInfo.freelancer.slice(0,10)}...</p>
          <p><strong>Amount:</strong> {escrowInfo.amount}</p>
          <p><strong>Status:</strong> <span style={styles.badge}>{escrowInfo.state}</span></p>
          {escrowInfo.state === 'Disputed' && (
            <div style={styles.btnRow}>
              <button style={{...styles.btn, background:'#22c55e'}} onClick={() => resolve(true)} disabled={loading}>
                Pay Freelancer
              </button>
              <button style={{...styles.btn, background:'#ef4444'}} onClick={() => resolve(false)} disabled={loading}>
                Refund Client
              </button>
            </div>
          )}
...
const styles = {
  ...
  heading: { color: '#7c3aed', borderBottom: '2px solid #ddd6fe', paddingBottom: '0.5rem' },
  ...
  btn: { background: '#7c3aed', color: 'white', border: 'none', padding: '0.6rem 1.2rem',
    borderRadius: '6px', cursor: 'pointer', fontSize: '0.9rem' },
  btnRow: { display: 'flex', gap: '0.5rem', marginTop: '1rem' },
  infoBox: { background: '#faf5ff', border: '1px solid #e9d5ff', borderRadius: '8px', padding: '1rem' },
  badge: { background: '#ede9fe', color: '#5b21b6', padding: '2px 8px', borderRadius: '4px', fontSize: '0.85rem' },
